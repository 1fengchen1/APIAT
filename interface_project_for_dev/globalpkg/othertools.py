...
class OtherTools:
    ...
    def conver_date_from_testlink(self, data):
        ...
        result_list = re.findall('<.?[^<]*>',
                                 data)  # 查找类似这类内容 <pre style="background-color:#ffffff;color:#000000;font-family:'宋体';font-size:12pt;">  并替换为空
        if result_list:
            for item in result_list:
                data = data.replace(item, '')
        ...
        for key_level in dict_level_list[self.key_index:len(dict_level_list) - 1]:
            ...
            if result == None:  # 如果还没遍历完成所有key就发现找不到对应层级的key，提前结束,节约时间
                return result
                    ...
                    []):  # 获取的值为列表，形如[{"goodsId":1,"goods_name":"apple"},{"goodsId":2,"goods_name":"apple"}]
                # 遍历列表,每个字典中查找
